        ...
        <div className="fade-in">
            ...
            {activities.length === 0 ? (
                ...
                activities.map((act) => {
                    ...
                    return (
                        <div key={act.id} className="card activity-card fade-in">
                            <span className={`audience-badge ${act.audience === 'public' ? 'audience-public' : 'audience-friends'}`}>
                                {act.audience === 'public' ? 'Public Record' : 'Internal Assembly'}
                            </span>
                            ...
                            <div className="activity-meta">
                                {act.location && <span className="activity-meta-item">LOCATION — {act.location}</span>}
                                <span className="activity-meta-item">SCHEDULED — {formatActivityTime(act.time)}</span>
                            </div>
                            ...
                            <div className="activity-actions">
                                ...
                                >
                                    Register Interest [{eng.interested?.count || 0}]
                                </button>
                                <button
                                ...
                                >
                                    {getJoinStatus(act.id) === 'pending' ? 'Application Under Review' :
                                        getJoinStatus(act.id) === 'accepted' ? 'Admission Granted' : 'Request Admission'}
                                </button>
